...
const ProductCard = ({ product, index = 0 }: ProductCardProps) => {
  ...
  return (
    ...
    >
      {/* Image */}
      <Link to={`/product/${product.id}`} className="block relative overflow-hidden aspect-[3/4] bg-muted/20">
        <img
          src={product.images[0]}
          alt={product.name}
          className="absolute inset-0 w-full h-full object-cover transition-all duration-700 group-hover:opacity-0 group-hover:scale-[1.02]"
          loading="lazy"
        />
        {product.images[1] && (
          <img
            ...
            alt={product.name}
            className="absolute inset-0 w-full h-full object-cover opacity-0 transition-all duration-700 group-hover:opacity-100 group-hover:scale-[1.02]"
            loading="lazy"
          />
        )}
      ...
      >
        <Heart
          size={13}
          className={`transition-all ${wishlisted ? "fill-foreground text-foreground scale-110" : "text-foreground/60"}`}
        />
      </button>
